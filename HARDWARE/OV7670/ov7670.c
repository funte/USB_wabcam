...
const u32 ov_maxbuf = OV_MAXBUF;
// #0摄像头缓冲区
OV_BUF		ovbuf0 = { 0, 0, 0 };
OV_BUF		ovbuf1 = { OV_MAXBUF, 0, 0};
u8			ov0_sta = 0;
...
u8 OV7670_Init (void)
{
	...
	GPIOA->CRH |= 0X00000008; 	// PA8 输入( 上拉/下拉输入模式 )  
	GPIOA->ODR |= 1 << 8;
	// WEN , RCLK 信号
	...
	GPIOB->ODR |= 3 << 3;
	// D[7:0] 数据信号
	GPIOC->CRL = 0X88888888; 	// PC0~7 输入(上拉下/拉输入模式)    
	GPIOC->ODR |= 0x00ff;
	// WRST 信号
	...
	// 初始化序列	  
	icount = sizeof( ov7670_init_reg_tbl ) / sizeof( ov7670_init_reg_tbl[0] );
	for (i = 0; i < icount; i++)
	{
...
void OV7670_CaptureFrame(void)
{
	...
	if (ov0_sta == 2)
	{
		...
		{
			OV7670_RCK = 0; color[1] = GPIOC->IDR & 0xff; OV7670_RCK = 1;
			OV7670_RCK = 0; color[0] = GPIOC->IDR & 0xff; OV7670_RCK = 1;
			// 写入像素
			FSMC_SRAM_WriteBuffer(color, povbuf->addr + i * 2, 2);
		}
		povbuf->bsize = elecount * 2;
		// 开始编码
		/*代码:JPG编码*/
		povbuf->sta = 1;
