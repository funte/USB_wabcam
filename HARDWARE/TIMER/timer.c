...
u8 USB_SPS; 			// 每秒发送帧数(编码速度+USB发送速度)
u8 SPSCount;
u8 OV0_FPS;				// #0摄像头刷新率(采集速度)
u8 FPS0Count;			
...
void TIM6_IRQHandler(void)
{
	if (TIM6->SR & 0X0001)//溢出中断
	{
		u8 col0xpos = 10;
		u8 col0width = 100;
		u8 col1xpos = col0xpos + col0width;
		...
		USB_SPS = SPSCount;
		SPSCount = 0;
		LCD_ShowString(col0xpos, 50, col0width, 16, 16, "USB_SPS:");
		LCD_ShowNum(col1xpos, 50, USB_SPS, 2, 16);
		
		OV0_FPS = FPS0Count;
		FPS0Count = 0;
		LCD_ShowString(col0xpos, 70, col0width, 16, 16, "OV0_FPS:");
		LCD_ShowNum(col1xpos, 70, OV0_FPS, 2, 16);

		LCD_ShowString(col0xpos, 90, col0width, 16, 16, "buf0:");
		LCD_ShowNum(10, 110, ovbuf0.addr, 10, 16);
		LCD_ShowNum(10, 130, ovbuf0.bsize, 10, 16);
		LCD_ShowNum(10, 150, ovbuf0.sta, 10, 16);
		LCD_ShowString(col0xpos, 170, col0width, 16, 16, "buf1:");
		LCD_ShowNum(10, 190, ovbuf1.addr, 10, 16);
		LCD_ShowNum(10, 210, ovbuf1.bsize, 10, 16);
		LCD_ShowNum(10, 230, ovbuf1.sta, 10, 16);
	}
	TIM6->SR &= ~(1 << 0);//清除中断标志位 	    
}
